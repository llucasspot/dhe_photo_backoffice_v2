...
const ProductDetailError = ({ error }: { error: Error | null }) => {
  const { t } = useI18n();
  const { getter } = useContextGetter(ProductGetter);
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div className="border-t border-gray-200">
          <dl>
            ...
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500">
                {t('dto.ProductDto.pictureFormatName.label')}
              </dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
...
export const ProductDetail = () => {
  const {
    queryResult: { data: product, isLoading, error },
  } = useContextGetter(ProductGetter);
