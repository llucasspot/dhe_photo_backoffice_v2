...
import { ProjectProducts } from './products/project-products.tsx';
import { FolderDropzone, KlassDropzoneHandlerService } from './index.ts';
...
const ProjectDetailError = ({ error }: { error: Error | null }) => {
  const { t } = useI18n();
  const { getter } = useContextGetter(ProjectGetter);
...
export const ProjectDetail = () => {
  const {
    queryResult: { data: project, isLoading, error },
  } = useContextGetter(ProjectGetter);
